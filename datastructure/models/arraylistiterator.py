...
Created on Tue Mar 19 09:58:31 2019
...
class ArrayListIterator(object):
    ...
    def previous(self):
        ...
        Returns the current item and moves the cursor
        to the previous item."""
        if not self.hasPrevious():
            raise ValueError("No previous item in iterator")
        if self._modCount != self._backingStore.getModCount():
            raise AttributeError(
                    "Illegal modification of backing store")
        self._cursor -= 1
    ...
    def replace(self, item):
        ...
        if self._lastItemPos == -1:
            raise AttributeError(
                    "The current osition is undefined.")
        if self._modCount != self._backingStore.getModCount():
            raise AttributeError(
